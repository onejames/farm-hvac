...
public:
    ...
    bool publish(const char* topic, const uint8_t* payload, size_t length) override {
        ...
        last_topic = topic;
        last_payload = std::string(reinterpret_cast<const char*>(payload), length);
...
void test_publish_sends_correct_json_payload(void) {
    ...
    // 2. Act
    bool result = publishMqttMessage(mockClient, AWS_IOT_TOPIC, data, test_version, test_build_date);
...
void test_publish_does_not_send_when_disconnected(void) {
    ...
    // 2. Act
    bool result = publishMqttMessage(mockClient, AWS_IOT_TOPIC, data, "v", "d");
